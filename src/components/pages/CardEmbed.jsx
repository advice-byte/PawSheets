...
export default function CardEmbed({ worksheet }) {
  const [styles, setStyles] = useState(null);

  useEffect(() => {
    if (!worksheet) return;
    if (worksheet.styles) setStyles(worksheet.styles);
  }, [worksheet]);

  if (!worksheet || !styles) return <div>Loading...</div>;

  // --- Filter rows with actual data (excluding header row) ---
  const validRows = (worksheet.rows || []).slice(1).filter((row) =>
    row.some((cell) => cell?.value && String(cell.value).trim() !== "")
  );

  if (!validRows.length) return <div>No data available for cards</div>;

  // --- Render card exactly like CardEditor.jsx ---
  const renderCard = () => {
    ...
    return (
      <div style={gridStyle}>
        {validRows.map((row, rIndex) => {
          ...
          return <div key={rIndex} style={containerStyle}>{content}</div>;
        })}
      </div>
    );
  };
  ...
  const iframeCode = `<iframe src="${window.location.origin}/embed/${worksheet.id}" style="border:none;width:100%;height:500px;"></iframe>`;

  // --- Generate full HTML embed code ---
  const htmlCode = `
<div style="
  display: ${styles.cardArrangement === "grid" ? "grid" : "flex"};
  ${styles.cardArrangement === "grid" ? `grid-template-columns: repeat(auto-fill, minmax(${styles.cardWidth}px, 1fr)); gap: ${styles.gap}px;` : `flex-direction: ${styles.cardArrangement === "row" ? "row" : "column"}; gap: ${styles.gap}px;`}
">
  ${validRows.map((row) => {
    const cells = (worksheet.columns || []).map((col, ci) => ({
    ...
    ">
      ...
      <div style="display:flex; flex-direction: column; gap:6px; flex:1;">
        ${fields.map(f => `<div style="display:flex; gap:4px; flex-wrap:wrap; align-items:baseline;"><span style="font-weight:700; font-size:${styles.fontSizeSecondary}px;">${f.header}:</span><span style="font-size:${styles.fontSizePrimary}px;">${f.value}</span></div>`).join("")}
      </div>
    </div>`;
  }).join("")}
</div>`.trim();
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div>
        <h3>Embed HTML:</h3>
        <textarea readOnly value={htmlCode} style={{ width: "100%", height: 300 }} onClick={(e) => e.target.select()} />
      </div>
      <div>
        <h3>Embed IFrame:</h3>
        <textarea readOnly value={iframeCode} style={{ width: "100%", height: 60 }} onClick={(e) => e.target.select()} />
      </div>
    </div>
  );
}
